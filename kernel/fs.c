...
bmap(struct inode *ip, uint bn)
{
  ...
  // change to work with checksums
  if(bn < NDIRECT){
    if((addr = ip->addrs[bn]) == 0) {
      ip->addrs[bn] = addr = balloc(ip->dev);
    }
    if(ip->type == T_CHECKED) {
      int new = 0x00ffffff & addr;
      return new;
    }
    return addr;
  }
...
stati(struct inode *ip, struct stat *st)
{
  ...
  int i;
  for(i = 0; i < NDIRECT; i++) {
    uint tmp = ip->addrs[i] >> 24;
    tmp  = tmp & 0xff;
    checksum = checksum ^ tmp;
  }

  struct buf *bp = bread(ip->dev, ip->addrs[NDIRECT]);
  uint *adr = (uint*)bp->data;
  int j;
  for(j = 0; j < NINDIRECT; j++) {
    uint tmp = (*adr) >> 24;
    tmp = tmp & 0xff;
...
/*
//Write checksum to the correct address.
static uint
check(struct inode *ip, uint bn, uint check)
{
  uint addr, *a;
  struct buf *bp;
  ...
  // change to work with checksums
  if(bn < NDIRECT){
    uint old = ip->addrs[bn];
    old = 0x00ffffff & old;
    check = (check << 24) & 0xff000000;
    ip->addrs[bn] = check + old;
    return 1;
  }
  bn -= NDIRECT;
  if(bn < NINDIRECT){
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    // PROJECT 5
    uint full; // CHECKSUM AND BLOCK NO.
    int num = -1;
    if(ip->type == T_CHECKED) {
      int i;
      for(i = 0; i < NDIRECT; i++) {
        int addr = 0x00ffffff & ip->addrs[i];
        if(addr == block) {
          num = i;
          full = ip->addrs[i];
          break;
      ...
      if(num == -1) {
        // INDIRECT BLOCKS
        uint addri = ip->addrs[NDIRECT];
        struct buf* b = bread(ip->dev, addri);
        uint *ptr = (uint*)b->data;
        int k;
        for(k = 0; k < NINDIRECT; k++) {
          int temp = 0x00ffffff & *ptr;
          if(block == temp) {
            num = k;
            full = *ptr;
            break;
      ...
      uint checksum = (full >> 24) & 0xff;
      char *sum = (char*) bp->data;
      // Calculate new checksum
      uint new_sum = 0;
      int g;
      for(g = 0; g < BSIZE; g++) {
        new_sum = new_sum ^ (uint)*sum;
        sum++;
      }
      new_sum = new_sum & 0xff;
      if(checksum != new_sum) {
        //ERROR 
        return -1;
      }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
	...
	struct buf* b;
      ...
      if(num == -1) {
        // INDIRECT BLOCKS
        int addri = ip->addrs[NDIRECT];
        b = bread(ip->dev, addri);
        uint *ptr = (uint*)b->data;
        indirect_ptr = ptr;
        int k;
        for(k = 0; k < NINDIRECT; k++) {
     ...
     // brelse(b);
      ...
      if(indirect != 1) {
        uint temp = new_sum << 24;
        ip->addrs[num] = temp+block;
        /************ Maybe add iupdate here ******************/
      }
      else {
        int f;
        for(f = 0; f < num; f++) {
